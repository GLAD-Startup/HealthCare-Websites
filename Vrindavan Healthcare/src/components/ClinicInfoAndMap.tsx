...
export const ClinicInfoAndMap: React.FC = () => {
  ...
  return (
    <section id="location" className="py-8 sm:py-16 lg:py-20 bg-[#FAFAF8] border-t border-slate-200/60">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-col sm:flex-row justify-center gap-2 sm:gap-3 mb-6 sm:mb-10 w-full max-w-xl mx-auto">
          {CLINIC_LOCATIONS.map((loc) => {
            ...
            return (
              <button
                ...
                className={`w-full sm:w-auto p-3 sm:p-4 rounded-xl sm:rounded-2xl border text-left transition-all cursor-pointer flex items-center gap-3 min-h-[44px] ${
                  isSelected
              ...
              >
                ...
                <div>
                  <div className={`text-[10px] sm:text-xs font-bold uppercase tracking-wider ${isSelected ? 'text-amber-300' : 'text-slate-500'}`}>
                    {loc.badge}
                  </div>
        ...
        <div className="grid lg:grid-cols-12 gap-6 sm:gap-8 items-start">
          ...
          <div className="lg:col-span-5 space-y-4 sm:space-y-6">
            ...
            <div className={`p-3.5 sm:p-4 rounded-2xl border flex items-center justify-between shadow-2xs ${
              isOpenToday
              ...
              <div className="flex items-center gap-2.5 sm:gap-3">
                ...
                <div>
                  <div className="text-[11px] sm:text-xs font-bold uppercase tracking-wider text-slate-700">
                    Today is {todayName}
                  </div>
              ...
              <span className="px-2.5 py-1 rounded-full bg-slate-900 text-white text-[9px] sm:text-[10px] font-bold uppercase tracking-wider">
                Active OPD
              </span>
            </div>

            {/* Selected Location Address Card */}
            <div className="p-4 sm:p-6 rounded-2xl bg-white border border-slate-200 shadow-2xs space-y-3 sm:space-y-4">
              <div className="flex items-start justify-between gap-2">
                <div>
                  <span className="text-[11px] sm:text-xs font-bold uppercase tracking-wider text-teal-900">
                ...
                <span className="px-2.5 sm:px-3 py-0.5 sm:py-1 rounded-full bg-[#FEF08A] border border-amber-300 text-[11px] sm:text-xs font-bold text-slate-900 shadow-2xs">
                  PIN 281121
                </span>
              </div>

              <div className="text-xs sm:text-sm text-slate-700 space-y-1 pt-1 border-t border-slate-100">
                ...
                <div className="text-[11px] sm:text-xs text-slate-600 pl-6">
                  Landmark: <strong>{selectedLocation.landmark}</strong>
                </div>
                <div className="text-[10px] sm:text-xs text-slate-500 pl-6">
                  DIGIPIN: <strong>{selectedLocation.digipin}</strong>
                </div>
              </div>
              ...
              <div className="pt-1 flex flex-wrap gap-2">
                <a
                  ...
                  className="flex-1 sm:flex-none justify-center px-4 py-2.5 rounded-full bg-slate-900 hover:bg-slate-800 text-white text-xs font-bold uppercase tracking-wider flex items-center gap-1.5 shadow-sm transition-all hover:scale-102 active:scale-98 min-h-[42px]"
                >
                ...
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="flex-1 sm:flex-none justify-center px-4 py-2.5 rounded-full bg-white hover:bg-slate-50 text-slate-800 border border-slate-200 text-xs font-bold flex items-center gap-1.5 shadow-2xs transition-all min-h-[42px]"
                >
                  <span>Mappls: {selectedLocation.mapplsPin}</span>
                  <ExternalLink className="w-3 h-3 ml-0.5" />
                </a>
              </div>
            </div>
          ...
          <div className="lg:col-span-7 flex flex-col">
            <div className="rounded-2xl sm:rounded-3xl border border-slate-200 overflow-hidden shadow-xs bg-white flex flex-col">
              ...
              {/* Native Leaflet Map Container: fills exactly 320px on mobile, 520px on desktop */}
              <div className="relative w-full h-[320px] sm:h-[520px] bg-slate-100 overflow-hidden">
                <div
                  ref={mapContainerRef}
